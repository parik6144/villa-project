<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        console.log('vvvvv', requiredInputs);
        ...
        const form = document.querySelector('form1111');
        if (form) {
        ...
        function basicDetailsValidation() {
            ...
            const reportableInput = document.getElementById('reportable-input');
            const propertyClassInput = document.getElementById('property-class-input');
            const propertyTypeInput = document.getElementById('data.property_type');
            const propertyTypeOtherInput = document.getElementById('data.title');
            const floorInput = document.getElementById('floorspace-input');
            const floorUnitInput = document.getElementById('floorspace-units-input');
            const basicRateCommissionIdInput = document.getElementById('basic_rate_commission_id');
            const rentalLicenceTypeInput = document.getElementById('rental-licence-type-input');
            const rentalLicenceNumberInput = document.getElementById('rental-licence-number-input');

            // check deal type
            const checkboxes = document.querySelectorAll('.deal-type-checkbox');
            let dealTypeInput = false;
            checkboxes.forEach(checkbox => {
                if (checkbox.checked && ['deal_type_rent', 'deal_type_monthly_rent'].includes(checkbox.value)) {
                    dealTypeInput = true;
                }
            });
            console.log('dealTypeInput', dealTypeInput);
            console.log('basicRateCommissionIdInput.value', basicRateCommissionIdInput.value);

            if (propertyClassInput.value != 'other') {
                document.getElementById('data.title').value = '';
            }

            let error = false;
            if (titleInput && titleInput.value.trim() === '' ||
                reportableInput && reportableInput.value.trim() === '' ||
                propertyClassInput && propertyClassInput.value.trim() === '' ||
                (propertyTypeInput && propertyTypeInput.value.trim() === '' &&
                    propertyTypeOtherInput && propertyTypeOtherInput.value.trim() === '') ||
                floorInput && floorInput.value.trim() === '' ||
                floorUnitInput && floorUnitInput.value.trim() === '' ||
                (dealTypeInput === true && basicRateCommissionIdInput && basicRateCommissionIdInput.value.trim() === '') ||
                (dealTypeInput === true && rentalLicenceTypeInput && rentalLicenceTypeInput.value.trim() === '') ||
                (dealTypeInput === true && rentalLicenceNumberInput && rentalLicenceNumberInput.value.trim() === '')) {
                error = true;
            } else {
                error = false;
            }
...
<!-- icon validation -->
<script>
    function iconvalidation(error = true) {
